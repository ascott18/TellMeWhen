...
TMW.Classes.GroupModule_GroupPosition:RegisterConfigTable("args.position.args", "Position", {
	...
	get = function(info)
		return TMW.db.profile.Groups[FindGroupIDFromInfo(info)].Point[info[#info]]
	end,
	dialogInline = true,
	guiInline = true,
...
TMW.Classes.GroupModule_GroupPosition:RegisterConfigTable("args.position.args", "scaleAndStrata", {
	...
	get = function(info) return TMW.db.profile.Groups[FindGroupIDFromInfo(info)][info[#info]] end,
	dialogInline = true,
	guiInline = true,
	...
	args = {
		...
		Strata = {
			...
			get = function(info)
				local val = TMW.db.profile.Groups[FindGroupIDFromInfo(info)][info[#info]]
				for k, v in pairs(stratas) do
					if v == val then
						return k
					end
