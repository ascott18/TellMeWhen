...
Type:RegisterConfigPanel_ConstructorFunc(100, "TellMeWhen_ValueSettings", function(self)
	...
	if TMW.ClassicExpansionAtLeast(LE_EXPANSION_CATACLYSM) 
	and TMW.ClassicExpansionAtMost(LE_EXPANSION_WARLORDS_OF_DRAENOR) then
		types[#types+1] = { order = 21, id = Enum.PowerType.ShadowOrbs,   name = SHADOW_ORBS }
	end

	-- Burning Embers & Demonic Fury: MoP through Warlords
	if TMW.ClassicExpansionAtLeast(LE_EXPANSION_MISTS_OF_PANDARIA) 
	and TMW.ClassicExpansionAtMost(LE_EXPANSION_WARLORDS_OF_DRAENOR) then
...
local comboPointsPerTarget = TMW.ClassicExpansionAtMost(LE_EXPANSION_MISTS_OF_PANDARIA)
...
function Type:Setup(icon)
	...
	if icon.PowerType == -4 then
		if TMW.ClassicExpansionAtLeast(LE_EXPANSION_LEGION) and pclass == "MAGE" and TMW.GetCurrentSpecialization() == SPEC_MAGE_ARCANE then
		...
		elseif pclass == "MONK" and TMW.GetCurrentSpecialization() == SPEC_MONK_WINDWALKER then
			...
			local ClassPowers = {
				PALADIN = TMW.ClassicExpansionAtLeast(LE_EXPANSION_CATACLYSM) and Enum.PowerType.HolyPower,
				WARLOCK = TMW.ClassicExpansionAtLeast(LE_EXPANSION_CATACLYSM) and Enum.PowerType.SoulShards,
	...
	if icon.UnitSet.allUnitsChangeOnEvent then
		...
		elseif icon.PowerType == -1 then
			icon:RegisterEvent(C_EventUtils.IsEventValid("UNIT_HEALTH_FREQUENT") and "UNIT_HEALTH_FREQUENT" or "UNIT_HEALTH")
			icon:RegisterEvent("UNIT_MAXHEALTH")
